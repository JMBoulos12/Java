...
/*
 * The Topological Sorting algorithm linearly orders a DAG or Directed Acyclic Graph into a linked list. 
 * A Directed Graph is proven to be acyclic when a DFS or Depth First Search is performed, yielding no back-edges.
    ...
    graph.adj
      ...
      .next.forEach(label -> {
        if (graph.adj.get(label).color == Color.WHITE) {
          ...
          l5ist.addFirst(sort(graph, graph.adj.get(label), list));
